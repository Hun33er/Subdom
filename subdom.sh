...
Usage(){
	...
	\r    -s, --silent       - The Only output will be the found subdomains - (Results saved: subdom-<DOMAIN>.txt).
	...
	\r	  wayback,crt,abuseipdb,bufferover,Findomain,Subfinder,Amass,Assetfinder,c99,alienvault,vedbex
	...
	\r    - To run subdom.sh against a list of domains:
	\r       $PRG -l domains.txt
...
wayback() {
	[ "$silent" == True ] && curl -sk "http://web.archive.org/cdx/search/cdx?url=*.$domain&output=txt&fl=original&collapse=urlkey&page=" | awk -F/ '{gsub(/:.*/, "", $3); print $3}' | sort -u | anew subdom-$domain.txt  || {
		[[ ${PARALLEL} == True ]] || { spinner "${bold}WayBackMachine${end}" &
			PID="$!"
		}
		curl -sk "http://web.archive.org/cdx/search/cdx?url=*.$domain&output=txt&fl=original&collapse=urlkey&page=" | awk -F/ '{gsub(/:.*/, "", $3); print $3}' | sort -u | anew tmp-wayback-$domain
...
c99() {
	[ "$silent" == True ] && curl -sk "https://subdomainfinder.c99.nl/scans/$current_date/$domain" | grep -o 'href="\/\/[a-zA-Z0-9.-]+\.$domain"' | grep -o '[a-zA-Z0-9.-]+\.$domain' | sort -u | anew subdom-$domain.txt || {
				[[ ${PARALLEL} == True ]] || { spinner "${bold}c99${end}" &
			PID="$!"
		}
		curl -sk "https://subdomainfinder.c99.nl/scans/$current_date/$domain" | grep -o 'href="\/\/[a-zA-Z0-9.-]+\.$domain"' | grep -o '[a-zA-Z0-9.-]+\.$domain'  | anew temp-c99-$domain
		[[ ${PARALLEL} == True ]] || kill ${PID} 2>/dev/null
		echo -e "$bold[*] c99$end: $(wc -l < tmp-c99-$domain)"
	}
}

alienvault() {
	[ "$silent" == True ] && curl https://otx.alienvault.com/otxapi/indicators/domain/passive_dns/$domain > | grep -oE '[^"]*\.$domain"); print $9}' | anew subdom-$domain.txt || {
		[[ ${PARALLEL} == True ]] || { spinner "${bold}Alienvault${end}" &
			PID="$!"
		}
		curl https://otx.alienvault.com/otxapi/indicators/domain/passive_dns/$domain | grep -oE '[^"]*\.$domain", $9); print $9}' | sort -u > tmp-alienvault-$domain
		[[ ${PARALLEL} == True ]] || kill ${PID} 2>/dev/null
...
vedbex() {
	[ "$silent" == True ] && curl https://www.vedbex.com/subdomain-finder/$domain > vedbex-$domain 
		grep -oP '(?<=<td>)[^<]*\.domain(?=</td>)' | anew subdom-$domain.txt || {
		[[ ${PARALLEL} == True ]] || { spinner "${bold}vedbex${end}" &
		PID="$!"
	}
		curl https://www.vedbex.com/subdomain-finder/$domain > vedbex-$domain.txt  
		grep -oP '(?<=<td>)[^<]*\.$domain(?=</td>)' temp-vedbex-$domain.txt | sort -u | anew temp-vedbex-$domain	
		[[ ${PARALLEL} == True ]] || kill ${PID} 2>/dev/null
		echo -e "$bold[*] Vedbex$end: $(wc -l < tmp-vedbex-$domain)"
	}
}

crt() {
	[ "$silent" == True ] && curl -sk "https://crt.sh/?q=%.$domain&output=json" | tr ',' '\n' | awk -F'"' '/name_value/ {gsub(/\*\./, "", $4); gsub(/\\n/,"\n",$4);print $4}' | anew subdom-$domain.txt || {
...
abuseipdb() {
	[ "$silent" == True ] && curl -s "https://www.abuseipdb.com/whois/$domain" -H "user-agent: firefox" -b "abuseipdb_session=" | grep -o '<li>[^<]*</li>' | sed 's/<\/\?li>//g' | sed -e "s/$/.$domain/" | anew subdom-$domain.txt || {
...
bufferover() {
	[ "$silent" == True ] && curl -s "https://dns.bufferover.run/dns?q=.$domain" | grep $domain | awk -F, '{gsub("\"", "", $2); print $2}' | anew subdom-$domain.txt || {
...
Findomain() {
	[ "$silent" == True ] && findomain -t $domain -q 2>/dev/null | anew subdom-$domain.txt || {
...
Subfinder() {
	[ "$silent" == True ] && subfinder -all -silent -config -d $domain 2>/dev/null | anew subdom-$domain.txt || {
		[[ ${PARALLEL} == True ]] || { spinner "${bold}SubFinder${end}" &
			PID="$!"
		}
		subfinder -all -silent -config ~/.config/subfinder/config.yaml -d $domain 1> tmp-subfinder-$domain 2>/dev/null
		[[ ${PARALLEL} == True ]] || kill ${PID} 2>/dev/null
...
Amass() {
	...
	[ "$silent" == True ] && amass enum -passive -norecursive -noalts -d $domain 2>/dev/null | anew subdom-$domain.txt || {
...
Assetfinder() {
	[ "$silent" == True ] && assetfinder --subs-only $domain | anew subdom-$domain.txt || {
	...
	while read domain; do
		...
		[ $prv == "a" ] && {
			[[ ${PARALLEL} == True ]] && {
				...
				parallel -j7 ::: wayback crt abuseipdb bufferover Findomain Subfinder Amass Assetfinder c99 alienvault vedbex
				kill ${PID}
				[[ $out != False ]] && OUT $out || OUT
			} || {
				wayback
	...
	[ $domain != False ] && { 
		[ $use == False ] && [ $exclude == False ] && { 
			[[ ${PARALLEL} == True ]] && {
				spinner "Reconnaissance" &
				PID="$!"
				export -f wayback crt abuseipdb bufferover Findomain Subfinder Amass Assetfinder spinner c99 alienvault vedbex
				export domain silent bold end
				parallel -j7 ::: wayback crt abuseipdb bufferover Findomain Subfinder Amass Assetfinder c99 alienvault vedbex
				kill ${PID}
			} || {
...
[ "$silent" == False ] && echo -e $red$bold"""

                __        __               
    _______  __/ /_  ____/ / _____  __  __ 
   / ___/ / / / __ \/ __  / / __  /|  \/  |
  (__  ) /_/ / /_/ / /_/ / / /_/ / | |\/| |
  /____/\__,_/_.___/\__,_/ \____/  |_|  |_|
 
           X: https://x.com/olamdeen
     Github: https://github.com/Hun333er
"""$end
